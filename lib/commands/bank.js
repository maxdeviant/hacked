...
if (operation === 'close') {
    ...
    BankAccount.findOne({
        ...
        if (err || account === null) {
            return io.emit(user.uuid + '-error', {
                message: escapeHtml('Account #' + account + ' not found.')
            });
        }

        if (account.pin !== pin) {
            return io.emit(user.uuid + '-error', {
                message: escapeHtml('Access denied' + account)
            });
        } else if (account.balance !== 0){
            return io.emit(user.uuid + '-error', {
                message: escapeHtml('Account balance is not $0')
            });
        } else {
            BankAccount.findOneAndRemove({number: new RegExp('^'+account.number+'$', "i")}, function(err, doc) {
                return io.emit(user.uuid + '-response', {
